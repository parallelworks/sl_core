...
# --groups '/pw/workflows/sl_test/whondrml_global_train_25_inputs.ixy'
# --backend 'loky'
#
# Caveats:
...
if __name__ == '__main__':
    ...
    # Always load and save the group data
    groups_df = pd.read_csv(args.groups)
    groups_df.to_csv(args.model_dir+'/original_group_data.csv')
    ...
    if args.smogn == "true":
        ...
        #---------------------------------------------------
        # Old, simple train_test_split, not group aware!
        #X_train, X_test, Y_train, Y_test = train_test_split(X, Y, random_state=SEED)
        #---------------------------------------------------
        # Group aware train-test split by first defining the
        # gss splitter object and then running the actual split.
        # next() is used because we have only one split.
        # Then, create separate dataframes by applying the
        # index values (*_idx) generated by the splitter.
        gss = GroupShuffleSplit(n_splits=1, test_size=0.25, random_state=SEED)
        train_idx, test_idx = next(gss.split(X, Y, groups=groups_df['gid']))
